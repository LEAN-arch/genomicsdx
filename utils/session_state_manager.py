    ...
    return {
        ...
        "risk_management_file": {
            ...
            "assay_fmea": [
                ...
                {"id": "AFM-02", "process_step": "Library Amplification", "failure_mode": "PCR contamination", "potential_effect": "False positive signal due to foreign DNA.", "S": 5, "O": 2, "D": 3, "mitigation": "Use physically separated pre/post-PCR areas; negative controls in each batch.", "verification_link": "PPQ-01"},
                {"id": "AFM-03", "process_step": "Reagent Handling", "failure_mode": "Use of expired/improperly stored reagent", "potential_effect": "Systematic assay failure; inaccurate results for entire batch.", "S": 4, "O": 2, "D": 1, "mitigation": "LIMS-based reagent inventory tracking with automated expiry alerts.", "verification_link": "LIMS-VAL-01"}
            ],
            "service_fmea": [
        ...
        "design_outputs": { "documents": [
                {"id": "SPEC-ASY-01", "type": "Assay Spec", "title": "Oligonucleotide Probe Sequences v1.2", "version": "1.2", "status": "Approved", "approval_date": str(base_date + timedelta(100)), "linked_input_id": "ASY-001", "link_to_artifact": "/docs/SPEC-ASY-01_v1.2.pdf"},
        ...
        "design_reviews": { "reviews": [
                ...
                {"id": "DR-003", "date": str(base_date + timedelta(365)), "attendees": team_list, "type": "Phase-Gate", "phase": "Algorithm Lock", "independent_reviewer": "Sofia Chen, PhD", "scope": "Review of classifier performance on training data. Approval to lock model for clinical validation.", "outcome": "Go with Conditions", "notes": "Performance is acceptable. Condition: Finalize Software V&V report before starting patient enrollment.", "action_items": [{"id": "AI-DR003-01", "description": "Complete and approve SW-VAL-01.", "owner": "Marcus Thorne, PhD", "due_date": str(base_date + timedelta(380)), "status": "In Progress", "risk_priority": "High", "voe_status": "N/A"}, {"id": "AI-DR003-02", "description": "Update risk file (H-04) with residual software risks.", "owner": "Jose Bautista", "due_date": str(base_date + timedelta(375)), "status": "Open", "risk_priority": "Medium", "voe_status": "N/A"}]}
            ]},
        ...
        "design_changes": {"changes": [
            {"id": "DCR-001", "description": "Change polymerase enzyme to improve library yield", "reason": "Observed lower than expected yield in pilot lots, impacting assay sensitivity.", "initiator": "Sofia Chen, PhD", "request_date": str(base_date + timedelta(120)), "impact_analytical": True, "impact_risk": True, "vv_plan": "Execute bridging study comparing old vs. new enzyme (AV-BRIDGE-01). Re-execute LoD study (AV-LOD-02).", "approval_status": "Closed", "approval_date": str(base_date + timedelta(150)), "approvers": ["Elena Reyes, PhD", "Jose Bautista"], "action_items": []}
        ]},
        "clinical_study": {
        ...
        "lab_operations": {
            ...
            "ppq_runs": [{"run_id": "PPQ-01", "description": "Run 1 of 3", "run_date": str(demo_current_date - timedelta(20)), "result": "Pass", "summary_report_link": "/reports/PPQ-01.pdf"}, {"run_id": "PPQ-02", "description": "Run 2 of 3", "run_date": str(demo_current_date - timedelta(10)), "result": "Pass", "summary_report_link": "/reports/PPQ-02.pdf"}, {"run_id": "PPQ-03", "description": "Run 3 of 3", "run_date": str(demo_current_date - timedelta(1)), "result": "In Progress", "summary_report_link": ""}],
        ...
        "quality_system": {
            "capa_records": [{"id": "CAPA-01", "status": "Closed", "source": "Internal Audit A-001", "description": "Procedure for reagent labeling was not consistently followed.", "action_plan": []}, {"id": "CAPA-02", "status": "Open", "source": "Supplier Corrective Action for IDT-Lot-123", "description": "High rate of oligo synthesis failure in a critical reagent lot from supplier IDT.", "action_plan": [{"id": "AI-CAPA02-01", "description": "Qualify new reagent lot from alt supplier", "owner": "Sofia Chen, PhD", "due_date": str(demo_current_date + timedelta(30)), "status": "Open", "risk_priority": "High", "voe_status": "N/A"}]}],
            "ncr_records": [{"id": "NCR-001", "status": "Closed", "description": "Temperature excursion in Refrigerator RF-02.", "correction_actions": [{"id":"AI-NCR01-01", "description":"Discard affected reagents", "owner":"Sofia Chen, PhD", "due_date": str(base_date+100), "status":"Completed", "risk_priority":"High", "voe_status":"N/A"}]}],
            "supplier_audits": [{"supplier": "IDT", "status": "Pass with Observations", "date": str(base_date + timedelta(20)), "report_id": "AUD-SUP-001"}, {"supplier": "Illumina", "status": "Pass", "date": str(base_date + timedelta(35)), "report_id": "AUD-SUP-002"}, {"supplier": "LabVantage", "status": "Pass", "date": str(base_date + timedelta(50)), "report_id": "AUD-SUP-003"}],
            "continuous_improvement": [{"date": str(demo_current_date - timedelta(d)), "ftr_rate": 95 - (d/30)*0.5, "copq_cost": 50000 - (d/30)*1000} for d in range(0, 181, 30)],
...
class SessionStateManager:
    ...
    _CURRENT_DATA_VERSION = 42 # Incremented for fully populated, corrected, and personalized data model
