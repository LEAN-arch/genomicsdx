...
def render_action_item_tracker(ssm: SessionStateManager) -> None:
    ...
    try:
        ...
        if open_items > 0:
            ...
            fig = px.bar(
                risk_status_summary, x="status", y="count", color="risk_priority",
                ...
                color_discrete_map={'High': '#d62728', 'Medium': '#ff7f0e', 'Low': '#1f77b4'}, text_auto=True
            )
        ...
        def style_row_by_risk(row: pd.Series) -> List[str]:
            color = ''
            if row.risk_priority == 'High': color = 'background-color: #ffcccc'
            elif row.risk_priority == 'Medium': color = 'background-color: #fff0cc'
        ...
        st.dataframe(
            ...
            column_config={
                "id": "Action ID", "description": st.column_config.TextColumn("Description", width="large"), "owner": "Owner",
                "due_date": st.column_config.DateColumn("Due Date", format="YYYY-MM-DD"), "status": "Status",
        ...
        st.download_button(label="📥 Export Filtered View to CSV", data=csv, file_name="compliance_actions_export.csv", mime="text/csv", key="export_compliance_actions")
